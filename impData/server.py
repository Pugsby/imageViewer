...
config = {
    "port": 8282,
    "imagesPath": os.path.normpath(os.path.join(SCRIPT_DIR, "..", "images")),
...
SCRAPER_SETTINGS = os.path.join(ROOT_DIR, "scraperSettings.json")

try:
    with open(VERSION_FILE, "r", encoding="utf-8") as f:
        serverVersion = f.read().strip()
except Exception:
    serverVersion = "idfk"
...
def check_for_updates():
    global lastVersionCheck, newVersion
    if time() - lastVersionCheck < 60:
        return False
    u = "https://raw.githubusercontent.com/Pugsby/imageViewer/refs/heads/main/version.txt"
    try:
        r = requests.get(u, timeout=10)
        if r.status_code == 200:
            latestVersion = r.text.strip()
            if latestVersion != serverVersion:
                newVersion = latestVersion
                print(f"Update available! Latest version is {latestVersion}, you are running {serverVersion}.")
    ...
    def do_GET(self):
        if self.path.startswith("/api"):
            ...
            if self.path.startswith("/api/search"):
                ...
                return
            elif self.path.startswith("/api/checkUpdates"):
                check_for_updates()
                self.send_response(200)
                self.send_header("Content-Type", "application/json")
                self.end_headers()
                print(serverVersion, newVersion)
                self.wfile.write(bytes(json.dumps({"newVersion": newVersion}), 'utf-8'))
                return
            elif self.path.startswith("/api/images"):
                # rawPath = self.path[len("images/"):] #what the fuck does this do
                # /api/images/e621/6351278.png = ../images/e621/6351278.png
                print(self.path)
                rawPath = self.path[len("/api/images/"):]
                queryString = ""
                if "?" in rawPath:
